...
def create_model():
    ...
    model = keras.Model(inputs=[uid, user_gender, user_age, user_job, user_interest, movie_id, movie_categories, movie_titles],
                        outputs=[inference])
    return model
...
# 两网络初始参数相同
current_net.save('current_net_weights.h5')
target_net.load_weights('current_net_weights.h5')
...
def select_action_and_change_e(ll)->int:
    ...
    current_epsilon -= (INITIAL_EPSILON - FINAL_EPSILON) / 2400000
    return ans + 1
...
# 分出训练集测试集 测试集占20%
train_X, test_X = features[:int(len(features) * 0.8)], features[int(len(features) * 0.8):]
train_Y, test_Y = targets_values[:int(len(targets_values) * 0.8)], targets_values[int(len(targets_values) * 0.8):]
...
for epoch_i in range(num_epochs):
    ...
    for i in range(len(train_X)):
        ...
        if len(replay_buffer) >= BATCH_SIZE and i%1000==0:#（测试：：减少训练时间）
