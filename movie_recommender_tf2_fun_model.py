...
import time
from tensorflow.keras.layers import Embedding,Dense,Concatenate,Conv2D,MaxPooling2D,Dropout,Flatten,Reshape,Input
...
def create_model():
    ...
    movie_categories_embed_layer = tf.keras.layers.Embedding(movie_categories_max, embed_dim, input_length=18)(movie_categories)
    ...
    model=keras.Model(inputs=[uid,user_gender,user_age,user_job,movie_id,movie_categories,movie_titles],outputs=[inference])
    return model
